...
describe('WACTFinalScoreSim', () => {
  let el: WACTFinalScoreSim;
  ...
  beforeEach(async () => {
    ...
    if (!customElements.get('wact-final-score-sim')) {
      customElements.define('wact-final-score-sim', WACTFinalScoreSim);
    }
    el = document.createElement('wact-final-score-sim') as WACTFinalScoreSim;
  ...
  it('should have the correct tag name', () => {
    expect(WACTFinalScoreSim.tagName).toBe('wact-final-score-sim');
  });
  ...
  it('should log error when simulate clicked without a service', async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      'No simulation service configured for wact-final-score-sim.',
    );
  });
  ...
  it('should display box score after successful simulation', async () => {
    ...
    const resultWrapper = el.root.getElementById(
      'final-score-sim__result-wrapper',
    ) as HTMLDivElement;
    expect(resultWrapper.style.display).toBe('block');

    const boxScore = el.root.getElementById('final-score-sim__result') as WACTBoxScore;
